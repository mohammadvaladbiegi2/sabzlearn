...
export default function DetailsCours({ course, user, Isbuycourse, islogin }) {
  ...
  const addCourseToPanel = async ([courseid, title, image, user]) => {
    ...
    let res = await fetch("https://sabzlearn-fde16fa99-mohammads-projects-2b3603e3.vercel.app/api/user/addcoursepanel", {
      method: "POST",
      headers: {
  ...
  const addNewcomment = async ([questioner, course]) => {
    if (commentText.length < 4) {
      ...
      let res = await fetch("https://sabzlearn-fde16fa99-mohammads-projects-2b3603e3.vercel.app/api/comment", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (res.status === 200) {
        ...
        fetch(`https://sabzlearn-fde16fa99-mohammads-projects-2b3603e3.vercel.app/api/course/${id}`).then(res => res.json()).then(data => {
...
export async function getServerSideProps(contex) {
  ...
  let res = await fetch(`https://sabzlearn-fde16fa99-mohammads-projects-2b3603e3.vercel.app/api/course/${contex.query.id}`);
